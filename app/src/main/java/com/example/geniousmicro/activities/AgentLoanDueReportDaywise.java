...
import com.example.geniousmicro.Adapter.AdapterLoanDueReport;
...
public class AgentLoanDueReportDaywise extends AppCompatActivity implements View.OnClickListener {
    // toolbar
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_agent_loan_due_report_daywise);
        ...
        mToolbarTitle.setText("Today Loan Collection");

        dialog = new ProgressDialog(AgentLoanDueReportDaywise.this);
...
//        System.out.println("Current date in dd-MM-yyyy format: " + formattedDate1);
    ...
    private void setSearchedLoanData(String s) {
        ArrayList<SetGetLoanDueReport> mTempArrayListDueReport = new ArrayList<>();
        for (SetGetLoanDueReport setGetLoanDueReport : mArrayListDueReport) {
        ...
        if (mTempArrayListDueReport.size() > 0) {
            adapterLoanDueReport = new AdapterLoanDueReport(AgentLoanDueReportDaywise.this, mTempArrayListDueReport);
            mRv_loanDueReport.setAdapter(adapterLoanDueReport);
            adapterLoanDueReport.notifyDataSetChanged();
        } else {
            ...
            if (cn != null) {
                ...
                if (rs.isBeforeFirst()) {
                    while (rs.next()) {
                        ...
                        setGetLoanDueReport.setPhNo(rs.getString("Loandate"));
                    ...
                    adapterLoanDueReport = new AdapterLoanDueReport(AgentLoanDueReportDaywise.this, mArrayListDueReport);
                    mRv_loanDueReport.setAdapter(adapterLoanDueReport);
                } else {
                    Toast.makeText(this, "No data found", Toast.LENGTH_SHORT).show();
                }
    ...
    public void getLoanDueReport(int fDate, int tDate, String memberCode) {
        ...
        try {
            if (cn != null) {
                CallableStatement smt = cn.prepareCall("{call ADROID_GetLoanDueReport_NEW(?,?,?)}");
                smt.setString("@ArrangerCode", GlobalUserData.employeeDataModel.getEmployeeID());
                smt.setInt("@Fdate", fDate);
                smt.setInt("@Tdate", tDate);
                ...
                if (rs.isBeforeFirst()) {
                    while (rs.next()) {
                        ...
                        setGetLoanDueReport.setTotalDueEmiAmnt(rs.getString("LoanAmount"));
                        setGetLoanDueReport.setNoOfPendingEMI(rs.getString("NoOfPendingEMI"));
                        mArrayListDueReport.add(setGetLoanDueReport);
                    }
                    adapterLoanDueReport = new AdapterLoanDueReport(AgentLoanDueReportDaywise.this, mArrayListDueReport);
                    mRv_loanDueReport.setAdapter(adapterLoanDueReport);
    ...
    private void selectFDate() {
        ...
        DatePickerDialog datePickerDialog = new DatePickerDialog(AgentLoanDueReportDaywise.this, new DatePickerDialog.OnDateSetListener() {
            @Override
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case android.R.id.home:
                ...
                startActivity(new Intent(AgentLoanDueReportDaywise.this, MainActivity.class));
